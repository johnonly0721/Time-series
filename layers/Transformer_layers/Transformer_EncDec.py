...
class EncoderLayer(nn.Module):
    def __init__(self, attention_layer, d_model, d_ff=None, dropout=0.1, activation='relu') -> None:
        super().__init__()
    ...
    def forward(self, x, attn_mask=None):
        x2, attn = self.attention_layer(x, x, x, attn_mask)
        x = x + self.dropout(x2)
        y = x = self.norm1(x)
...
class DecoderLayer(nn.Module):
    def __init__(self, self_attention, cross_attention, d_model,
                 d_ff=None, dropout=0.1, activation='relu') -> None:
        super().__init__()
        d_ff = d_ff or 4 * d_model
