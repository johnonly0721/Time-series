...
from layers.Patch_TST.PatchTST_layers import *
...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, 
                 ...
                 subtract_last:bool=False, verbose:bool=False, **kwargs) -> None:
        ...
        self.backbone = TSTiEncoder(c_in=c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len, 
                                    ...
                                    learn_pe=learn_pe, verbose=verbose, **kwargs)
        ...
        if self.pretrain_head:
            self.head = self.create_pretrained_head(self.head_nf, self.n_vars, fc_dropout)
    ...
    def create_pretrained_head(self, head_nf, vars, dropout):
        return nn.Sequential(nn.Dropout(dropout),
                             nn.Conv1d(head_nf, vars, 1))


class Flatten_Head(nn.Module):
    def __init__(self, individual, n_vars, nf, target_window, head_dropout=0.) -> None:
        super().__init__()
    ...
    def forward(self, x):  # x: [bs x nvars x d_model x patch_num]
        if self.individual:
            ...
            for i in range(self.n_vars):
                ...
                z = self.dropouts[i](z)
                out.append(z)
            x = torch.stack(out, dim=1) # x: [bs x nvars x target_window]
        else:
            x = self.flatten(x)
            x = self.linear(x)
            x = self.dropout(x)
...
class TSTiEncoder(nn.Module):  # i 代表 通道独立
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 ...
                 pre_norm=False, pe='zeros', learn_pe=True, verbose=False, **kwargs) -> None:
...
class TSTEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None,
                 ...
                 res_attention=False, n_layers=1, pre_norm=False, store_attn=False) -> None:

        super().__init__()
        
        self.layers = nn.ModuleList([TSTEncoderLayer(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff,
                                                     norm=norm, attn_dropout=attn_dropout, dropout=dropout,
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
                 norm='BatchNorm', attn_dropout=0., dropout=0., bias=True, activation='gelu', res_attention=False, pre_norm=False) -> None:
        super().__init__()
...
class _MultiheadAttention(nn.Module):
    def __init__(self, d_model, n_heads, d_k=None, d_v=None, res_attention=False, attn_dropout=0., proj_dropout=0., qkv_bias=True, lsa=False) -> None:
        super().__init__()
        d_k = d_model // n_heads if d_k is None else d_k
        d_v = d_model // n_heads if d_v is None else d_v
...
class _ScaledDotProductAttention(nn.Module):
    def __init__(self, d_model, n_heads, attn_dropout=0., res_attention=False, lsa=False) -> None:
        super().__init__()
